...
namespace SKYNET.Managers
{
    public static class APIClient
    {
        ...
        public static void GoOffline()
        {
            ...
            // Runs before the shutdown signal, so this last call is still allowed.
        ...
        /// <summary>
        /// Cancels every in-flight synchronous request so the workers blocked
        /// in GetResponse() return immediately instead of being torn down by
        /// the CLR while the game exits.
        /// </summary>
        public static void AbortPendingRequests()
        {
            foreach (var pending in InFlightRequests.Keys)
            {
                try
                {
                    pending.Abort();
        ...
        private static void PresenceDispatchLoop()
        {
            while (!Lifetime.IsShuttingDown)
            {
                try
                {
                    if (!Lifetime.Wait(PresenceSignal, 250))
                    {
                        return;
                    }
        ...
        private static readonly ConcurrentDictionary<HttpWebRequest, bool> InFlightRequests =
            new ConcurrentDictionary<HttpWebRequest, bool>();
        ...
        private static SyncHttpResponse HttpRequestSync(string method, string path, string jsonBody, bool applyAuth, int timeoutMs)
        {
            ...
            {
                throw new OperationCanceledException("Emulator is shutting down");
            }
            ...
            {
                InFlightRequests.TryRemove(request, out _);
            }
        }
        ...
        private static void P2PDispatchLoop()
        {
            while (!Lifetime.IsShuttingDown)
            {
                try
                {
                    if (!Lifetime.Wait(P2PQueueSignal, 25))
                    {
                        return;
                    }

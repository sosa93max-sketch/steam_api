...
namespace SKYNET.Helper
{
    public class Settings
    {
        ...
        /// <summary>
        /// Machine-wide gate around the read-modify-write of steam_api.ini.
        /// Several processes share one game folder (client, dedicated server,
        /// a second launch), and without it two of them interleave load and
        /// save and one writes back what it parsed from a half-written file.
        /// </summary>
        private const string ConfigMutexName = @"Global\D2MAX_steam_api_ini";
        ...
        public static void Load()
        {
            ...
            try
            {
                configMutex = new Mutex(false, ConfigMutexName);
                try
                {
                    mutexOwned = configMutex.WaitOne(TimeSpan.FromSeconds(10));
                }
                catch (AbandonedMutexException)
                {
                    // A process died holding it; the file is still consistent
                    // because every write is atomic.
                    mutexOwned = true;
                }
            }
            catch
            {
                // Global\ needs SeCreateGlobalPrivilege, which a sandboxed game
                // process may lack; a session-local mutex still covers the case
                // that matters (two launches by the same user).
                try
                {
                    configMutex = new Mutex(false, @"Local\D2MAX_steam_api_ini");
                    mutexOwned = configMutex.WaitOne(TimeSpan.FromSeconds(10));
                }
                catch
                {
        ...
        {
            // Verify Paths
            Common.EnsureDirectoryExists(Common.DataPath());
            Common.EnsureDirectoryExists(Common.DataPath("Storage"));
            Common.EnsureDirectoryExists(Common.DataPath("AvatarCache"));

            try
            {
                string fileName = Common.DataPath("steam_api.ini");

                // Migrate an older "[SKYNET] steam_api.ini" to the new name so
                // existing installs keep their configuration.
                string legacyFileName = Common.DataPath("[SKYNET] steam_api.ini");
                ...
                if (!File.Exists(fileName))
                {
                    ...
                    config.AppendLine("ServerUrl = http://127.0.0.1:5199/");
                    ...
                    INIParser.WriteAtomic(fileName, config.ToString());
                }
            ...
            {
                // Never block the game's startup with a modal dialog: the load
                // path runs while the game is loading the DLL.
                SteamEmulator.Write("Settings", $"Settings load failed: {ex}");
            }
        ...
        private static bool EnsureMissingDefaults()
        {
            ...
            changed |= MigrateSettingName("Network Settings", "SkyNetServerUrl", "ServerUrl", "http://127.0.0.1:5199/");
        ...
        private static void LoadDLCs()
        {
            try
            {
                ...
                string dlcFile = Common.DataPath("DLC.txt");
                string legacyDlcFile = Common.DataPath("[SKYNET] DLC.txt");
        ...
        private static void LoadAppContent()
        {
            try
            {
                ...
                string skynetPath = Common.DataPath();
